...
locAdd := function(P1, P2, f, flag)
  ...
  if d1 + d2 gt g+1 then error "Sum of degrees of P1 and P2 is larger than g+1"; end if;
  ...
  elif (flag eq false) and (b1 ne Parent(b1)!0) then newb := ChineseRemainderTheorem([b1, b2], [a1, a2]);
...
//Comment: g must not be a square
//!!!Change in number of arguments
findlocPt := function(f, g, WPts, rand_hits, inf)
  //print f, g, WPts, rand_hits;
  kt := Parent(f); 
  k := BaseRing(kt);
  l := #WPts;
  WPts := [k!t : t in WPts]; 
  ...
  if inf eq true then 
    ...
    signchanges := [];
    for i in [1..l-1] do 
      if g[i]*g[i+1] eq -1 then Append(~signchanges, i); end if;
    end for;
    if signchanges eq [] then return true,[kt!1, kt!0]; end if;
    ...
    dega := Degree(a);
    M := Matrix(k, dega, dega, [[x[1]^(dega-i) : i in [1..dega]]: x in pts]);
    M := Transpose(M);
    vec := Vector(k, dega, [x[2] : x in pts]);
    coef := Solution(M,vec); 
    b := &+[kt.1^(dega-i)*coef[i]: i in [1.. dega]];
    return true, [a,b];
    ...
    if forall{t : t in g | IsSquare(t)} then return true, [k!1, k!0]; end if;// the point at infinity. 
  ...
  // Search for a point on the curve first.
    ...
    // if x is a Weierstrass point then skip the iteration
    ...
    // curve, therefore skip the iteration.
      if not IsSquare(Evaluate(f,x)) then continue; 
        ...
        if flag then continue;
          ...
          Include(~SqSet, <[x, SquareRoot(Evaluate(f, x))], <x-WPts[t]: t in [1..l]>>);
          subSqSet := Subsets(SqSet);
          flag, seq := MemberUptoSq(subSqSet, g);
          if flag then 
            for t in seq do bool, newJcPt, fctn := locAdd([kt.1- t[1][1], kt!(t[1][2])], newJcPt,f, true);
            end for;
...
/*
  ...
  */
   ...
   */
    ...
    //coef_a[1] := coef_a[1]/pi; 
    a := kt.1^genus+kt!Polynomial(coef_a); //to get the parent of
    // a and f in the same Polynomial ring to avoid illegal coercions later.
    d := Gcd(a, Derivative(a)); 
    //make a square free (maybe I need to put it below for a faster
    //algorithm
    //a := &*[fct[1]^(fct[2] mod 2): fct in Factorization(a)];
    while Degree(d) ne 0 do 
      a := a div d; d := Gcd(a, Derivative(a));
    end while;
    //IsUnramified(SplittingField(a));
//print <(-1)^Degree(a)*Evaluate(a,WPts[t])/g[t]: t in [1..l]>;
    //Check if corresponds to a local point for g 
    if forall{t : t in [1..l] | IsSquare((-1)^Degree(a)*Evaluate(a,WPts[t])/g[t])} 
    then
      if not locPolEq(GCD(f,a), kt!1) then 
      //print "gcd issue"; 
      continue; end if; //find a local point which does 
                                                  //not have a Weierstrass
                                                  //point in the support.
      fctrs_a := [ai[1]: ai in Factorization(a)];
...
EvalFuncPairing1 := function(g, Pt,f, flag)
  ...
  if flag and (Pt[2] eq Parent(Pt[1])!0) then d := Gcd(a, D); a := a div d; D := D div d; 
...
IsPrincipalDivisor := function(c,Pv,Pvs,f)
  ...
  f_old :=[P!1, P!1];
  cnew := c; prod := Kt!1; 
  for j in cnew do if Degree(prod) + deg eq genus then break; end if;
    prod := prod * (Kt.1 - j);
    Exclude(~cnew, j);
  end for;
  bool, newPt, g := locAdd([prod, Kt!0], Pv, f, true);
  ...
  //note that at some here in the above line I have used the fact that P=Q iff bar(P)=bar(Q).
    return true, [f_old[1]*g, f_old[2]]; //Need to correct it for local equalities
  elif cnew eq {} then 
    return false, _;
  end if;
  ...
  while IsEmpty(cnew) eq false do 
  j := Random(cnew);
  //print "size of c: ", cnew;
  flag, newPt, g := locAdd([Kt.1-j, Kt!0], newPt, f, true);
  //print "exited locAdd with return flag : ", flag;
  assert flag eq true;
  g := y - emb(g);
  f_old := [f_old[1]*g, f_old[2]*emb(newPt[1])];
  Exclude(~cnew, j);
  end while;
 // return newPt;
...
Comp1Cocyc := function(m, H, WPts, mumrep, i, f, sqrtg, Pg)
  ...
  chi_val := [[*c[1],zero diff SequenceToSet(Indices(c[2], false))*] : c in H_sqrtg];
  ...
  for c in chi_val do
    ...
    flag, g := IsPrincipalDivisor({WPts[j]: j in c[2]}, mumrep, mumrepc, f);
    assert flag eq true;
    //return g;
    ...
    for pt in JacPt do
      ...
      if pt[1] eq [1,0,0] then
        ...
        val := val*EvalFuncPairing1(CompMinRep(g[1], g[2], f, true), pt[1], f, true)^(pt[2]);
      end if; 
    end for;      
      // Ti + newPt + Q := div(y-g)
...
// represented by P and the WP's is g+1 and then keep adding extra points sequentially. 
    ...
    eps_val, pseq, sseq := comp_epsilon(c[2], i, 1);
    //print "comp_epsilon returned:", eps_val, pseq, sseq;
    if pseq ne [] then
    eps_val := eps_val *(&*[Pg[t[1]][t[2]]^(t[3]): t in pseq]); end if;
    if sseq ne [] then
    eps_val := eps_val *(&*[(WPts[t[1]]-WPts[t[2]])^(t[3]): t in sseq]); end if;
    //print Norm(eps_val);
    val := val/eps_val;
...
CompDiagVal := function(Pv, sig, sqrt, i, WPts, Pg, f)
  a := Pv[1]; b := Pv[2]; deg := Degree(a); l := #WPts; genus := l div 2;
  zero := SequenceToSet([1..l]);
  chi_val := SequenceToSet(Indices([locEltEq(sig(d),d): d in sqrt],true));
     
  k := Domain(sig);
  //print "chi_val : ", chi_val;
  eps_val, pseq, sseq := comp_epsilon(chi_val, i, -1);
  //print "comp_epsilon returned : ", eps_val, pseq, sseq;
  eps_val := k!eps_val;
  if pseq ne [] then  
  ...
  if chi_val eq zero and Parent(a)!b eq Parent(a)!0 then
  ...
  if chi_val eq zero then 
    ...
    flag, g := IsPrincipalDivisor({WPts[x]: x in chi_val}, Pv, Pvs,f);
    assert flag eq true;
    P := Parent(g[1]);
    x := P.1 ; y := P.2; embed := hom<kt->P| x>; pi := hom<P -> kt| [kt.1, 0]>; 
    //evaluating g at Ti
...
IsrealPrincipalDivisor := function(c,Pv, f)
  l := Degree(f); genus := l div 2;  
                                  // if chi_val is full i.e. eq zero then sigma is trivial
                                  // there is no action on Pv i.e. Pvs eq Pv and divisor 
                                  // c + Pv -Pvs= 0 = div(1);
  ...
  bool, newPt, g := locAdd([prod, Kt!0], Pv, f, false);
  assert bool eq false; 
  //print "first new point : ",newPt; 
  ...
  j := Random(cnew);
  //print "size of c: ", cnew;
  flag, newPt, g := locAdd([Kt.1-j, Kt!0], newPt, f, false);
  //print "exited locAdd with return flag : ", flag;
  assert flag eq true;
  g := y - emb(g);
  f_old := [f_old[1]*g, f_old[2]*emb(newPt[1])];
  ...
  return true, f_old, newPt; //Need to correct it for local equalities
end function;
