  ...
  for i := 1 to #ff do
	...
	oll := MaximalOrder(LL);
        ...
        test := function(s)
		  ...
		  Ii:= ideal<oll|di>;
		  Ij:= ideal<oll|dj>;
		  si:= Support(Divisor(Ii));
		  sj := Support(Divisor(Ij));
		  primes_i := [Ideal(p): p in si];
		  primes_j := [Ideal(p): p in sj];
		  &cat[primes, primes_i, primes_j];
...
function is_good(C)
  ...
  good := good_subgroup(SStors, SGtoA);
  return SG, TinSG, SSinSG, good;
end function;
