...
M. Stoll, 2017-09-22:
(1) Improved handling of intermediate supersets of the Selmer set by
    representing them as a union of cosets of a subgroup of the ambient group
    instead of the full set (which can be prohibitively large in the early
    stages).
(2) Added "Points" optional argument that can be set to contain a set of
    rational points on the curve, to be used for an early exit when the
    given points exhaust the upper bound for the Selmer set computed so far.

M. Stoll, 2021-05-27:
Fixed a bug in TwoCoverDescentInternal: When the degree of the defining
polynomial is odd and the leading coefficient is not a square, the image
of the point at infinity on the curve in the Selmer set was computed wrongly.

 ******************************************************************************/

import "selmer.m" : IsFactorizationPrecisionError;

// New (M. Stoll, 2017-09-22)
// Update the pair of variables representing a subset of H1S
function UpdateDeltas(cs, sg, h, im)
  // cs is a subset, sg a subgroup
  //   of a finite ambient abelian group G.
  // The pair represents the set
  //   cs + sg = {c + s : c in cs, s in sg}.
  // h is a homomorphism G --> G', im a subset of G'.
  // returns new csnew and sgnew such that
  //  csnew + sgnew = {d in cs + sg : h(d) in im}.
  sgnew := sg meet Kernel(h); // new subgroup
  reps := Transversal(sg, sgnew); // (indexed) set of coset reps of sgnew in sg
  csnew := {g : c in cs, r in reps | h(g) in im where g := c+r};
  return csnew, sgnew;
end function;
...
intrinsic TwoCoverDescentInternal(
			  ...
			  // Added (M. Stoll, 2017-09-22); here Points is a set of x-coordinates
			  Points := {})
  ...
  if lc in Image(H1norm) then
    LC:=lc@@H1norm;
    // Changed (M. Stoll, 2017-09-22)
    // (Replace deltas with deltascosets and deltassubgroup, where
    //  deltas = {c + s : c in deltascosets, s in deltassubgroup};
    //  deltascosets is a subset of H1S, deltassubgroup a subgroup.)
    deltascosets := {LC};
    deltassubgroup := Kernel(H1norm);
//     deltas:={LC+delta:delta in Kernel(H1norm)};
    vprint Selmer:"After Norm criterion, #deltas <=",#deltascosets*#deltassubgroup;
  else
    // No change necessary here (M. Stoll, 2017-09-22)
    deltas:={};
    vprint Selmer:"Norm criterion rules out any deltas";
    return deltas,toH1S,toVec,bU;
  end if;

  // Moved the following to after the determination of the first upper bound
  // (M. Stoll, 2021-05-27).
  // Added (M. Stoll, 2017-09-22)
  // Compute image of given points
  lowerbound := {H1S|};
  if not IsEmpty(Points) then
    // get infinity as an element of the universe of Points
    infty := [Universe(Points)| Infinity()][1];
  end if;
  for xc in Points do
    vprintf Selmer: "xc = %o\n", xc;
    if xc eq infty then
      // Changed 2021-05-27, Michael Stoll:
      // need to take into account shift by the leading coefficient
      Include(~lowerbound, toH1S(LeadingCoefficient(F)));
//       Include(~lowerbound, H1S!0);
    elif Evaluate(F, xc) eq 0 then
      // have to patch, see Bruin-Stoll, page 2350
      cofactor := ExactQuotient(F, Parent(F).1 - xc);
      if IsEven(Degree(F)) then
        // scale to avoid denominator, which can lead to an error in toH1S
        Include(~lowerbound, toH1S(Denominator(xc)*((xc-theta) + Evaluate(cofactor, theta))));
      else
        Include(~lowerbound, toH1S((xc-theta) + Evaluate(cofactor, theta)));
      end if;
    else
      if IsEven(Degree(F)) then
        // scale to avoid denominator
        Include(~lowerbound, toH1S(Numerator(xc)-Denominator(xc)*theta));
      else
        Include(~lowerbound, toH1S(xc-theta));
      end if;
    end if;
  end for;
  vprintf Selmer: "Lower bound for Selmer set has size %o\n", #lowerbound;
...
******************/
  ...
  for pl in [p: p in InfinitePlaces(K) | IsReal(p)] do
    ...
    // Changed (M. Stoll, 2017-09-22)
    deltascosets, deltassubgroup := UpdateDeltas(deltascosets, deltassubgroup, H1StoRH1, L);
//     deltas:={delta:delta in deltas| H1StoRH1(delta) in L};
    vprint Selmer:"Now, #deltas <=",#deltascosets*#deltassubgroup;
    if #deltascosets eq 0 then
      vprint Selmer:"No deltas left. Shortcut return.";
      return deltascosets,toH1S,toVec,bU;
    // Added (M. Stoll, 2017-09-22)
    elif #deltascosets*#deltassubgroup eq #lowerbound then
      assert lowerbound eq {c+s : c in deltascosets, s in deltassubgroup};
      vprintf Selmer: "Current set covered by known points. Shortcut return.";
      return lowerbound,toH1S,toVec,bU;
    end if;
  end for;
  ...
  for P in Primes do
    ...
    repeat
      ...
      // Changed (M. Stoll, 2017-09-22)
      imsubgroup := Lmap(deltassubgroup);
      imcosets := {Lmap(c) : c in deltascosets};
      target := {c+s : c in imcosets, s in imsubgroup};
//       target:={Lmap(delta):delta in deltas};
    ...
    // Changed (M. Stoll, 2017-09-22)
    deltascosets, deltassubgroup := UpdateDeltas(deltascosets, deltassubgroup, Lmap, LocalDeltas);
//     deltas:={delta:delta in deltas| Lmap(delta) in LocalDeltas};
  ...
  // Changed (M. Stoll, 2017-09-22)
  return {c+s : c in deltascosets, s in deltassubgroup},toH1S,toVec,bU;
end intrinsic;

intrinsic TwoCoverDescent(C::CrvHyp:
     Bound:=-1,Fields:=false,Raw:=false,PrimeBound:=0,PrimeCutoff:=0,
     // Added (M. Stoll, 2017-09-22); SearchLimit = Bound for point search
     Points := false, SearchLimit := 10000)
  -> SetEnum, Map, Map, SeqEnum
  {"} // "
  ...
  // Added (M. Stoll, 2017-09-22)
  // set up universe for elements of the base field K and infinity
  // (<--> elements of P^1(K) <--> x-coordinates of points on C)
  univ := cop<BaseRing(C), Parent(Infinity())>;
  if Points cmpeq false then
    if Type(BaseRing(C)) eq FldRat then
      // over Q, do a point search to set this argument
      xcoords := {univ| pt[3] eq 0 select Infinity() else pt[1]/pt[3]
                      : pt in RationalPoints(HyperellipticCurve(F) : Bound := SearchLimit)};
      vprintf Selmer: "Found %o pairs of rational points on C.\n", #xcoords;
    else
    ...
    // check
    require Universe(Points) cmpeq C(BaseRing(C)):
     "Points must be points on the curve";
    // x-coordinates are unchanged by move to simplified model
    xcoords := {univ| pt[3] eq 0 select Infinity() else pt[1]/pt[3]
                      : pt in Points};
  end if;
