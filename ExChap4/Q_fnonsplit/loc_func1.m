...
findlocPt := function(f, g, WPts, rand_hits)
  ...
  newJcPt := [kt!1,kt!0];
  for degree in [1..(l div 2)] do 
    Ext_degree := AllExtensions(Ok, degree);
    for E in Ext_degree do 
      for i in [1..rand_hits] do
      x := Random(E); a := kt!MinimalPolynomial(x, Ok); 
      if Degree(a) ne degree then continue;end if;
...
//Since g is not in the image of 2-torsion, hence skip if x matches a Weierstrass point
    ...
    // curve, therefore skip the iteration.
      if not IsSquare(Evaluate(f,x)) then continue;
        ...
        if flag then continue;
          ...
          lE := LocalField(k, a);
          b := kt!Polynomial(ElementToSequence(SquareRoot(Evaluate(f,lE.1))));
    ...
    end for; //rand_hits iteration
  end for; //extension of particular degree iteration
end for; //degree iteration
return false, _;
/*
...
#SqSet;
  ...
  for degree in [2..genus] do
  for i in [1..rand_hits] do 
    coef_a := [k!Random(Ok): i in [1..degree]];
    a := kt.1^degree+kt!Polynomial(coef_a);
