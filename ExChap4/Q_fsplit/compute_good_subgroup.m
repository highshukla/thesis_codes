...
function SS_and_T_in_SG(C, SScomp)
  ...
  TinSG := sub<SG | [x_minus_T(TtoJ(t), f, A, AtoSG) : t in OrderedGenerators(T)]>;
  if #TinSG eq #SG then 
...
function is_good(C, SScomp)
  ...
  try 
  good, good_elts := good_subgroup(SStors, SGtoA);
  catch e
  print HyperellipticPolynomials(C);
  return 0;
  end try;
  return (#quo<SG|good> eq 1), SG, TinSG, SSinSG, SStors, AtoSG, SGtoA, good, good_elts;
end function;
