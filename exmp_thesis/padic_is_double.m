...
function pAdicIsDouble(f,pt)
  ...
  P := Parent(f); x := P.1;
  Qv := BaseRing(P);
  if Degree(pt[1]) eq 0 then 
    if Roots(f) ne [] then
    return true, [P.1-Roots(f)[1][1], P!0]; 
    else return false,_;
    end if;
  end if;
  ...
  rootf := [-Evaluate(e, Qv!0): e in ffact]; //roots of f
  ...
  */
  ...
  if Degree(d) eq 0 then
    ...
    s := locCR([sqrts[i] @@ algs[i][3] : i in [1..#algs]], ffact);
    //return Gcd(s^2-aa,  f);
    
    //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    assert locPolEq((s^2 - aa) mod f, P!0);//need to correct for precision for this
            // //assertion
    //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!        
    ...
    q *:= 1/lcq; u *:= 1/lcq; v *:= 1/lcq;
     assert locPolEq(u^2*f, v^2 - aa*q^2);
...
//     printf "gcd(a, f) = %o\n", d;
    ...
    s := locCR([sqrts[i] @@ algs[i][3] : i in inds2], ffact[inds2]);
    sp := locCR([sqrts[i] @@ algs[i][3] : i in inds1], ffact[inds1]); // s'
     assert locPolEq((s^2 - sign*a) mod f1,P!0);
     assert locPolEq((sp^2 + sign*a1*f1) mod d, P!0);
    df1 := Degree(f1);
    da1 := Degree(a1);
    ...
    lcq := LeadingCoefficient(q);
    q *:= 1/lcq; u *:= 1/lcq; v *:= 1/lcq;
     assert locPolEq(u^2*f1, v^2*d - sign*a1*q^2);
    gcd, r1 := XGCD(u, q);
    q1 := ExactQuotient(q, gcd); u1 := ExactQuotient(u, gcd); v1 := ExactQuotient(v, gcd);
    if Degree(gcd) eq 0 then
      r := (-d*v*r1) mod q;
