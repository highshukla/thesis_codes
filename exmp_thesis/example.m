load "/home/himanshu/thesis_codes/exmp_thesis/search_fast_ms_org.m";
load "/home/himanshu/thesis_codes/exmp_thesis/glob_func.m";
load "/home/himanshu/thesis_codes/exmp_thesis/loc_func.m";
load "/home/himanshu/thesis_codes/exmp_thesis/padic_is_double.m";
ZZ := Integers();
QQ := Rationals();
...
globmats := [*CompGlobMat(ff, SGtoA(good_elts[i]), WPts, G, m): i in [1..#good_elts-1]*];
PoBR := SequenceToSet(PrimeDivisors(2*Integers()!Discriminant(f)));
ctpval := 1;
//change this later just for testing 

for a in [1..#good_elts-1] do 
  print "primes for computation: ", PoBR join globmats[a][2];
  for v in PoBR join globmats[a][2] do
...
*/
  ...
  for ap in [a+1..#good_elts] do
    gp := SGtoA(good_elts[ap]); 
    for h in &cat ffv do
      lh := LocalField(Qv,h); 
      if IsSquare(Evaluate(gp,lh.1)) then continue; end if;
      i := 0;
      ...
      */
      ...
      pr := quo<IntegerRing(Kv)|UniformizingElement(Kv)^(ramindKvlh*Precision(lh)-20)>;
      ...
      ctpval *:= pAdicHilbSym(mh(DelAtdp), Evaluate(gp, mh(lh.1)));
      catch e
      Lh,mh := RamifiedRepresentation(lh); 
      {pAdicHilbSym(mh(rtsdel[1]), Evaluate(gp, mh(lh.1))) eq pAdicHilbSym(mh(rts), Evaluate(gp, mh(lh.1))): rts in rtsdel};
      ctpval *:= pAdicHilbSym(mh(rtsdel[1]), Evaluate(gp, mh(lh.1)));
      end try;
      end if;
...
sqrtg := <Cf!1: i in [1..7]>;
for i in [1,4,5] do 
sqrtg[i]:= SquareRoot(g[i]);
end for;
sqg := SquareRoot(g[2]); sqrtg[2] := sqg; sqrtg[3] := ComplexConjugate(sqg);
sqg := SquareRoot(g[6]); sqrtg[6] := sqg; sqrtg[7] := ComplexConjugate(sqg);
...
for i in [1,4,5] do 
  if gp[i] eq -1 then
    eps_val := 1; 
    pseq :=  [<i,j,-1>: j in [1..7]| j ne i];
    eps_val := eps_val *(&*[(sqrtg[t[1]]*StoC(u[1]) + sqrtg[t[2]]*StoC(u[2]))^(t[3]) where u := Gmat[t[1]][t[2]] : t in pseq]);
    Sign(Real(eps_val));
    end if;
end for;
*/
